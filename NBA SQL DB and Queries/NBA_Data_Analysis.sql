...
/*Determine the number of players classified as "stretch bigs" (centers or power forwards with three-point attempt rate above 20%) and the percentage of total big-man records they represent per era. Eras being grouped by seasons 1980-1999 Era 1, 2000-2012 era 2, 2015-2025 era*/
...
/*Calculate the average 3 points scored and it's share in toal points for the entire league by era. Eras being grouped by seasons 1980-1999 Era 1, 2000-2012 era 2, 2013-2025 era*/
...
/*Top 5 teams in 3 point attempts in 1987, 1996, 2015 vs that season champions(1987 - Los Angles Lakers, 1996 - Chicago Bulls, and 2015 - Golden State Warriros) in 3 point attempts*/
...
/*Examine the distribution of lineup net ratings for lineups that include at least three players with a three-point attempt rate above 30%.*/

With Player_3pt_Flag As (
Select
a.player_id,
a.team_id,
a.season,
...
pt.p_total_mp,
Case
When a.three_attempt_rt > 0.30 Then 1 Else 0 End As is_high_3pt_rate
From Advanced a
Inner Join Player_Totals pt
On  a.player_id = pt.player_id
And a.team_id   = pt.team_id
And a.season    = pt.season
Where pt.p_total_mp > 0
And a.three_attempt_rt Is Not Null
),
Lineup_Base As (
Select
team_id,
...
Order By p_total_mp Desc) As mp_rank
From Player_3pt_Flag
),
Top5_Lineup As (
Select *
From Lineup_Base
...
Sum(is_high_3pt_rate) As high_3pt_players,
Round(Sum(bpm * p_total_mp) / Nullif(Sum(p_total_mp), 0), 2) As lineup_net_rating,
Round(Sum(offensive_bpm * p_total_mp) / Nullif(Sum(p_total_mp), 0), 2) As lineup_off_rating,
Round(Sum(defensive_bpm * p_total_mp) / Nullif(Sum(p_total_mp), 0), 2) As lineup_def_rating,
Round(Avg(three_attempt_rt), 4) As avg_3pt_attempt_rate
From Top5_Lineup
Group By team_id, season, team
)
Select
season As Season,
team As Team,
high_3pt_players As Players_Above_30pct_3PA,
avg_3pt_attempt_rate As Avg_3pt_Attempt_Rate,
lineup_off_rating As Lineup_Off_Rating,
lineup_def_rating As Lineup_Def_Rating,
lineup_net_rating As Lineup_Net_Rating,
Case
When lineup_net_rating >=  10 Then 'Elite  (≥ +10)'
When lineup_net_rating >=   5 Then 'Good   (+5 to +10)'
When lineup_net_rating >=   0 Then 'Average (0 to +5)'
When lineup_net_rating >=  -5 Then 'Below avg (-5 to 0)'
Else 'Poor (< -5)'       
End As Net_Rating_Bucket
From Lineup_Summary
Where high_3pt_players >= 3
Order By season, lineup_net_rating Desc;
